...
public class UniNotes {
        ...
        do {
            ...
            switch (scelta) {
                case 1:System.out.println("Inserisci il link dell'appunto: \n");
                    ...
                    break;
                case 2: listaAcquisti.add(utenteCorrente.acquistaAppunto(elencoFacolta,elencoCarteDiCredito,elencoUtenti));
                    break;
    ...
    public LinkedList<Materia> materiePerFacolta(Integer codiceFacolta) throws Exception {
        ...
        if (listaMaterie.size() == 0) {
            ...
            if (scelta == 1)
                listaAcquisti.add(utenteCorrente.acquistaAppunto(elencoFacolta,elencoCarteDiCredito,elencoUtenti));
            else
                menuUtente();
        }
        return listaMaterie;
            ...
            if (facoltaEntry.getValue().getListaMaterie().size() != 0) {
                ...
                for (Materia m : listaMaterie) {
                    if (m.getElencoAppuntiInSospeso().size() != 0) {
                        Map<String, Appunto> elencoAppuntiInSospeso = m.getElencoAppuntiInSospeso();
    ...
    public void registrazione(String nickName, String nome, String cognome, String email, String password, String carta, String scadenza) throws IOException {
        ...
        FileWriter file1=new FileWriter("C:\\Users\\Enrica Spataro\\Dropbox\\Magistrale\\Progetto Tomarchio\\Elaborazione\\UniNotes-IT4\\src\\carteDiCredito.txt");
